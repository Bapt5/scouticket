...
- **Route groups**: `(auth)` = sign-in/forgot-password/reset-password pages; `(main)` = the authenticated app shell including `gestion-membres` (member management) and `gestion-unites` (unit management), both owner/admin-only.
