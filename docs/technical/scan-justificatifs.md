...
Le paramètre est stocké dans `scouticket_group_data` (migration `sql/008_parametres_groupe.sql`, à appliquer avec `pnpm db:migrate`) :
...
- Après une mise à jour, appliquez la migration `008` (`pnpm db:migrate`) : sans elle, la lecture des paramètres échoue.
